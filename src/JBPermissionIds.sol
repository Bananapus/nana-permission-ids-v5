...
/// @notice Permission IDs for `JBPermissions`, used throughout the Bananapus ecosystem. See https://github.com/Bananapus/nana-core/blob/main/src/JBPermissions.sol
/// @dev `JBPermissions` allows one address to grant another address permission to call functions in Juicebox contracts on their behalf. Each ID in `JBPermissionIds` grants access to a specific set of these functions.
library JBPermissionIds {
    ...
    uint256 internal constant QUEUE_RULESETS = 2; // Permission to call `JBController.queueRulesetsOf` and `JBController.launchRulesetsFor`.
    ...
    uint256 internal constant ADD_ACCOUNTING_CONTEXTS = 20; // Permission to call `JBMultiTerminal.addAccountingContextsFor`.
    ...
    uint256 internal constant SET_BUYBACK_TWAP = 25; // Permission to call `JBBuybackHook.setTwapWindowOf` and `JBBuybackHook.setTwapSlippageToleranceOf`.
    ...
    uint256 internal constant ADD_SWAP_TERMINAL_TWAP_PARAMS = 28; // Permission to call `JBSwapTerminal.addTwapParamsFor`.
